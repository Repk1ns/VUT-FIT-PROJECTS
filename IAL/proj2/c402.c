...
**/
    ...
    {
        tBTNodePtr new_node = NULL;
        tBTNodePtr pom = (*RootPtr);

        while (new_node == NULL)
        {
            if (Content < pom->Cont) // Levy podstrom
            {
                if (pom->LPtr != NULL) // Kontrola leveho podstromu
                {
                    pom = pom->LPtr;
                }

                else // Levy podstrom neni, tak ho vytvorime
                {
                    if((new_node = malloc(sizeof(struct tBTNode))) == NULL) // alokace
                    {
                        return;
                    }
            ...
            else if (Content > pom->Cont) // Pravy podstrom
            {
                if (pom->RPtr != NULL) // Kontrola praveho podstromu
                {
                    pom = pom->RPtr;
                }

                else // Pravy podstrom neni, tak ho vytvorime
                {
                    if((new_node = malloc(sizeof(struct tBTNode))) == NULL) // alokace
                    {
                        return;
...
**/
	...
	{
       	pom = STopPopP (&stack_P);
		SPushP (&stack_P, pom);

        pom_bool = STopPopB(&stack_B);
		if(pom_bool)
		{
			SPushB(&stack_B, false);
			Leftmost_Postorder(pom->RPtr, &stack_P, &stack_B);
		}
...
**/
	...
	Leftmost_Inorder(pom, &stack_P);

	while(!SEmptyP(&stack_P))
    {
		pom = STopPopP(&stack_P);

		Leftmost_Inorder(pom->RPtr, &stack_P);
