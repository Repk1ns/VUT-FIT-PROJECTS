...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
	...
	while(!stackEmpty(s) && (stackTop(s, &pom_top), pom_top) != '(')
	{
		stackTop(s, &pom_top);
		stackPop(s);
		postExpr[*postLen] = pom_top;
		(*postLen)++;
	}
	stackPop(s);
...
char* infix2postfix (const char* infExpr) {
	...
	while((c = infExpr[i]) != '\0')
	{
		if ((infExpr[i] >= 'a' && infExpr[i] <= 'z') || (infExpr[i] >= 'A' && infExpr[i] <= 'Z') || (infExpr[i] >= '0' && infExpr[i] <= '9')) 
		{
			post_Expr[post_Index] = infExpr[i];
			post_Index++;
		}
		else if(infExpr[i] == '+' || infExpr[i] == '-' || infExpr[i] == '*' || infExpr[i] == '/')
		{
			doOperation(&stack, c, post_Expr, &post_Index);
		}
		else if(infExpr[i] == ')')
		{
			untilLeftPar(&stack, post_Expr, &post_Index);
		}
		else if(infExpr[i] == '(')
		{
			stackPush(&stack, '(');
		...
		else if(infExpr[i] == '=')
		{
			...
			{
				stackTop(&stack, &post_Expr[post_Index]);
				post_Index++;
				stackPop(&stack);
			}
			post_Expr[post_Index] = '=';
			post_Expr[post_Index+1] = '\0';
		}
		i++;
	}
	return post_Expr;
